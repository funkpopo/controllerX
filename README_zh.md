...
适用于 Xbox、PlayStation 手柄以及键盘鼠标的透明桌面按键显示工具。

[English README](README.md)

controllerX 会在桌面上显示悬浮输入图层。当你按下手柄按键、移动摇杆、扣动扳机、敲击键盘或使用鼠标时，图层会同步变化。它适合录制游戏、直播展示、测试输入设备，以及检查手柄是否被正确识别。

## 主要功能

- 透明、置顶的桌面悬浮窗口。
- 支持 Xbox 360、Xbox One、Xbox Series、DualShock 3、DualShock 4、DualSense，以及常见 XInput 兼容手柄。
- 显示键盘鼠标图层，包括固定 64 键紧凑配列和鼠标按键。
- 托盘菜单支持显示/隐藏、鼠标穿透、锁定位置、语言选择和退出。
- 自动保存语言、透明度、窗口位置、死区、灵敏度、轴反转等设置。
- 在 Windows 上，即使窗口失去焦点或开启鼠标穿透，手柄和键鼠状态也可以继续更新。
